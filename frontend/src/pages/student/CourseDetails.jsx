...
const CourseDetails = () => {
  ...
  return courseData ? (<>
    <div className="flex min-h-[100vh] md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left">
      ...
      <div className="max-w-xl text-gray-500 z-10">
          ...
          <div className="flex">
            {[...Array(5)].map((_, index) => (
              <img
                ...
                key={index}
                src={
                  index < Math.floor(courseRating(courseData))
                    ? assets.star
                    : assets.star_blank
                }
                alt="star"
        ...
        <p className="text-sm">
          Course by <span className="text-blue-600 underline">Harsh Kumar</span>
        </p>
              ...
              <div key={index} className="border border-gray-300 mb-2 rounded bg-white">
                ...
                <div className={`overflow-hidden ${openSection[index]?"max-h-96":"max-h-0"} transition-all duration-300`}>
                  <ul className="list-disc pl-4 md:pl-10 pr-2 py-2 text-gray-600 border-t border-gray-300">
                    {chapter.chapterContent.map((lecture,index )=>(<li className="flex items-start py-1 gap-2" key={index}>
      ...
      <div className="max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]">
          ...
          {playerData?
          <YouTube iframeClassName="w-full aspect-video" videoId={playerData.videoId} opts={{playerVars:{
            autoplay:1,
          }}}/>:
        ...
        <div className="p-5">
          <div className="flex items-center gap-2">
            <img className="w-3.5" src={assets.time_left_clock_icon} alt="clock_icon" />
            <p className="text-red-500"> <span className="font-medium">5 days</span> left at this price</p>
          </div>
          <div className="flex items-center gap-4 pt-2">
            <p className="text-gray-800 text-2xl md:4xl font-semibold">{currency} {(courseData.coursePrice - courseData.discount *courseData.coursePrice/100).toFixed(2)} </p>
            <p className="md:text-xl text-gray-500 line-through">{currency} {courseData.coursePrice}</p>
            <p className="md:text-xl text-gray-500">{courseData.discount}% off </p>
          </div>
          <div className="flex items-center text-sm md:text-default gap-4 pt-2 md:pt-4 text-gray-500">
            ...
            <div className="flex gap-2 items-center">
              ...
              <p> {courseTime(courseData)} </p>
            </div>
            <div className="h-4 w-px bg-gray-500"></div>
            <div className="flex gap-2 items-center">
              <img src={assets.lesson_icon} alt="Lesson icon" />
              <p> {totalLectures(courseData)} Lessons </p>
            </div>
          </div>
          ...
          <div className="pt-6">
            ...
            <ul className="text-sm ml-4 pt-2 md:text-default list-disc text-gray-500">
